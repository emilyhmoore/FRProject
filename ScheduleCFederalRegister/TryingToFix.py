		...
		for k in range(len(MultiLineItems)):
			for j in range(len(arule)):
				...
				##
					...
					stuffIWantAsOne=" ".join(stuffIWant)
					count=stuffIWantAsOne.count(MultiLineItems[k])
					if count>1:
						joined=" ".join(stuffIWant[-count:])
						del stuffIWant[-count:]
						stuffIWant.append(joined)
		...
		for k in range(len(LastLineItems)):
			for j in range(len(arule)):
				...
				if re.search(LastLineItemsEND[k], arule[j])!=None:
					...
					string=re.split("\s{2,}",arule[start:end][-1]) #trying this instead of below
